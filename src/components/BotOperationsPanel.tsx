...
import { Activity, AlertTriangle, Award, Database, Radio, RefreshCw, Scale, ShieldCheck, Siren, Sparkles, TerminalSquare } from 'lucide-react';
import { BacktestResult, SupportedAsset, TradeRecord } from '../types';
import { getBotAdminHeaders } from '../utils/botAdminAuth';
import { SignalSuccessMetricsPanel } from './SignalSuccessMetricsPanel';
import { GeminiInsightsPanel } from './GeminiInsightsPanel';
import { ExecutionGapComparisonPanel } from './ExecutionGapComparisonPanel';
...
export const BotOperationsPanel: React.FC<BotOperationsPanelProps> = ({
  lang,
  currentAsset,
  paperTrades = [],
  backtestResult,
}) => {
  const [opsSubView, setOpsSubView] = useState<'gap' | 'insights' | 'metrics' | 'daemon'>('gap');
  ...
  return (
    <div className="space-y-4">
      {/* Sub-tab Navigation for Operations Center */}
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-3 bg-[#0d0d0d] p-2 rounded-lg border border-[#1f1f1f]">
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => setOpsSubView('gap')}
          ...
          >
            ...
            <span>{lang === 'ar' ? 'التحليل المقارن (الفعلي vs الباك تيست)' : 'Side-by-Side Execution Gap'}</span>
          </button>

          <button
            onClick={() => setOpsSubView('insights')}
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded font-mono text-xs font-bold transition-all ${
              opsSubView === 'insights'
          ...
          >
            ...
            <span>{lang === 'ar' ? 'دروس مستفادة (Gemini Flash 3.8)' : 'Actionable Insights (Gemini 3.8 Flash)'}</span>
          </button>

          <button
            onClick={() => setOpsSubView('metrics')}
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded font-mono text-xs font-bold transition-all ${
              opsSubView === 'metrics'
          ...
          >
            ...
            <span>{lang === 'ar' ? 'مصفوفة مقاييس النجاح (48h)' : 'Success Metrics Matrix (48h)'}</span>
          </button>

          <button
            onClick={() => setOpsSubView('daemon')}
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded font-mono text-xs font-bold transition-all ${
              opsSubView === 'daemon'
          ...
          >
            ...
            <span>{lang === 'ar' ? 'مراقبة السيرفر واللوجز المباشرة' : 'Server Daemon & Live Logs'}</span>
          </button>
        </div>

        <div className="text-[11px] font-mono text-gray-400 px-1 hidden xl:block">
          {opsSubView === 'gap'
            ? (lang === 'ar' ? 'تحليل مقارن دقيق لفجوات الانزلاق السعري بين التنفيذ الحقيقي والباك تيست النظري' : 'Side-by-side comparative analysis of slippage gaps and execution latency vs backtest')
            : opsSubView === 'insights'
            ? (lang === 'ar' ? 'تحليل نوعي ذكي للأسباب الفنية لنجاح أو تفادي الصفقات' : 'Deep AI analysis of why signals won or exited based on volatility')
            : opsSubView === 'metrics'
            ? (lang === 'ar' ? 'تدقيق كمي دقيق لتطور أسعار الصفقات ومعدل النجاح' : 'Quantitative audit of signal accuracy vs. actual price moves')
            : (lang === 'ar' ? 'مراقبة الفاحص الآلي في الخلفية وسجلات الحماية' : 'Live daemon monitoring and secure runtime logs')}
        </div>
      </div>

      {opsSubView === 'gap' ? (
        <ExecutionGapComparisonPanel
          lang={lang}
        ...
        <SignalSuccessMetricsPanel
          lang={lang}
          currentAsset={currentAsset}
          persistedSignals={signals}
          lastKnownPrices={daemon?.lastKnownPrices}
        />
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div className="bg-[#0a0a0a] border border-[#1f1f1f] rounded-xl p-4 space-y-3">
              ...
              <div className="space-y-2 max-h-[520px] overflow-auto pr-1">{filteredSignals.length === 0 ? <div className="rounded-lg border border-dashed border-[#333] p-4 text-sm text-gray-500 text-center">{lang === 'ar' ? 'لا توجد إشارات محفوظة بعد لهذا الأصل.' : 'No persisted signals yet for this asset.'}</div> : filteredSignals.map((signal) => <div key={signal.id} className={`rounded-lg border p-3 ${signal.isGateBlocked ? 'text-amber-300 border-amber-500/30 bg-amber-950/15' : signalColors[signal.signalType] || signalColors.HOLD}`}><div className="flex items-start justify-between gap-3"><div><div className="font-bold text-sm flex items-center gap-2"><span>{signal.signalType} • {signal.spotAction}</span>{signal.isGateBlocked && <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30">🛡️ {signal.regimeGateStatus || 'BLOCKED'}</span>}</div><div className="text-[11px] opacity-80">{new Date(signal.timestamp).toLocaleString()}</div></div><div className="text-right font-mono text-xs"><div>{lang === 'ar' ? 'الثقة' : 'Confidence'}: <span className="font-bold text-white">{signal.convictionScore}%</span></div><div>{lang === 'ar' ? 'السعر' : 'Price'}: <span className="font-bold text-white">${signal.price.toLocaleString()}</span></div></div></div><div className="grid grid-cols-2 gap-2 mt-3 text-[11px] font-mono"><div className="rounded border border-black/20 bg-black/10 p-2">Entry: <span className="text-white font-bold">${signal.entryPrice.toLocaleString()}</span></div><div className="rounded border border-black/20 bg-black/10 p-2">SL: <span className="text-white font-bold">${signal.stopLoss.toLocaleString()}</span></div><div className="rounded border border-black/20 bg-black/10 p-2">TP1: <span className="text-white font-bold">${signal.target1.toLocaleString()}</span></div><div className="rounded border border-black/20 bg-black/10 p-2">TP2: <span className="text-white font-bold">${signal.target2.toLocaleString()}</span></div></div>{signal.blockReason && <div className="mt-2 text-[11px] text-amber-300/90 font-mono bg-amber-950/30 rounded p-1.5 border border-amber-500/20">🛡️ Filter: {signal.blockReason}</div>}<div className="mt-3 text-xs leading-6 text-gray-100">{lang === 'ar' ? signal.summaryAr : signal.summaryEn}</div></div>)}</div>
            </div>
